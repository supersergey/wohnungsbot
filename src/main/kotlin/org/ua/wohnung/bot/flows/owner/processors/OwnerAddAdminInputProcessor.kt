...
class OwnerAddAdminInputProcessor(
    ...
    override val supportedStep = FlowStep.OWNER_ADD_ADMIN
    ...
    override fun processSpecificCommands(chatMetadata: ChatMetadata): StepOutput {
        return accountService.findByUsername(chatMetadata.input)?.let {
            accountService.updateUserRole(it.id, Role.ADMIN)
            StepOutput.PlainText(
                message = messageSource[FlowStep.OWNER_ADD_ADMIN_DONE].format(it.username),
